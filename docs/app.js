// Virtual Root — in-browser auxin transport model (port of auxin_v2.py) + teaching UI:
// anatomy labels, reflux flux arrows, mutant/gravitropism presets, animated bending, PNG export.
// Two compartments (cell + apoplast wall, walls at quasi-steady state); per-side,
// carrier-modulated permeabilities (Grieneisen um/s); Band PIN/AUX1 maps; rounded tip.
...
function rebuild(){
  ...
  for (let r=0;r<R;r++) for (let c=0;c<C;c++){
    ...
    if(rootward) ed=P;
    if(shootward && !params.pin2ko) eu=P;              // PIN2 knockout removes shootward pumps
    if(m.epi[i]&&r>=r_mer){ if(c<c0) er=P; else if(c>c0) el=P; }
    if(m.col[i]){ eu=ed=el=er=P; if(params.gravity){ el=P*1.6; er=P*0.4; } }  // gravitropic bias
...
// ---- colour map (gamma, auxin spans orders of magnitude) ----
const STOPS=[[0,0,4],[59,15,112],[140,41,129],[222,73,104],[254,159,109],[252,253,191]], GAMMA=0.45;
...
function render(){
  ...
  for(let r=0;r<R;r++) for(let c=0;c<C;c++){ const i=r*C+c; if(!active[i])continue;
    ctx.fillStyle=magma(a[i]/max); ctx.fillRect(OX+c*PX,OY+r*PX,PX,PX); }

  if(params.arrows){                                   // reflux flux arrows
  ...
  document.getElementById('maxinfo').textContent =
    (mi>=0?`auxin max ${max.toFixed(1)} µM at the ${(mi/C|0)>=r_qc-1&&Math.abs(mi%C-c0)<=2?'QC ✓':'row '+(mi/C|0)}`:'')
    + (params.gravity?` · gravistimulated (asymmetry ${(Math.abs(asym)*100).toFixed(0)}%)`:'');
}
...
function setPreset(name){ Object.assign(params,PRESETS[name]);
